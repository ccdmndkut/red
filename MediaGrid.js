...
function MediaGrid({ posts, onOpenMedia, gridSize = 240, getMediaInfo }) {
    ...
    const explodeView = (item, event) => {
        event.stopPropagation();
        // Toggle the exploded state for this item
        setExplodedPostId(currentExplodedId =>
            currentExplodedId === item.id ? null : item.id
        );

        // Prevent body scrolling when an item is exploded
        if (explodedPostId !== item.id) {
    ...
    // Make MediaGrid available globally
